...
my $rules = $ipt_obj->chain_rules('filter', 'OUTPUT');# select FILTER table and OUTPUT part of the status.log file.
...
<title>outgoing traffic - firewall web panel</title>
</head>
...
#     
   for (my $i = 0; $i < ($size);  ++$i) # output array's element start from 0 to $size
# in compare to forwarding.pl and incomming.pl there should be some thing wrong in IPtable::Parse module that cause this difference. 
    {
     my $rule_num = 1 + $i; #rule number = array's index +1.
     print "<tr><td>" . ($rule_num) . "</td>\n";
     print "<td>" . $rules->[$i]->{"src"} . "</td>\n";
     print "<td>" . $rules->[$i]->{"dst"} . "</td>\n";
     print "<td>" . $rules->[$i]->{"intf_out"} . "</td>\n";
