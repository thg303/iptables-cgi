...
sub new() {
	...
	_ipt_file   => $args{'ipt_file'} || '', # logfile's path.
        _debug     => $args{'debug'}     || 0,
...
sub chain_rules() {
	...
	#default example: no comment :)
        $rule{'comment'} = ""; # define comment, required if there was no comment
	my $comment = $rule{'extended'};  
	#gather comment from extended field (if exists) and fix extened's content.      
	if ($rule{'extended'} =~ /.*? \/\* .*? \*\/ .*?/) { # example: tcp spt:23 dpt:12 /* test is 13 23 */ reject-with icmp-port-unreachable
           $comment =~ s/.*? \/\* //;
           $comment =~ s/ \*\/ .*//;
           $rule{'comment'} = $comment;
           $rule{'extended'} =~ s/ \/\* .*? \*\///;
        } elsif ($rule{'extended'} =~ /\/\* .*? \*\/ .*?/) { # example: /* yet another comment  */ reject-with icmp-port-unreachable
	   $comment =~ s/\/\* //;
           $comment =~ s/ \*\/ .*//;
	   $rule{'comment'} = $comment;
           $rule{'extended'} =~ s/\/\* .*? \*\/ //;
	} elsif ($rule{'extended'} =~ /\/\* .*? \*\//) { # example: /* yet another comment  */
	   $comment =~ s/\/\* //;
           $comment =~ s/ \*\///;
	   $rule{'comment'} = $comment;
           $rule{'extended'} = "";
